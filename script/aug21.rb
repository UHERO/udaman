...
# from the api, annual is just an avg of the data points
...
ids.each_with_index do |id, idx|
  ...
  series_attrs = {
    :xseries_attributes => xs_attrs,
    :name_parts => name_parts,
    :dataPortalName => "COMBINED #{regions[idx]}",
    :description => "",
    :unit_id => unit_id,
    :source_id => bls_source_id,
    :source_link => "",
  ...
  # set up load statement
  # monthly series
  eval_str = "BUG#{id}@#{id.include?("S49FSA0") ? "HI" : "US"}.M"
  ...
  eval_str = "BUG#{semi_id}@#{id.include?("S49FSA0") ? "HI" : "US"}.S"
  eval_str = "\"#{eval_str}\".ts"
  ds_attrs = {
    :eval => eval_str,
    :priority => 50,
...
ids.each_with_index do |id, idx|
  ...
  # set up load statement
  eval_str = "BUG1#{id}@#{id.include?("S49FSA0") ? "HI" : "US"}.M"
...
ids.each_with_index do |id, idx|
  ...
  # set up load statement
  eval_str = "BUG2#{id}@#{id.include?("S49FSA0") ? "HI" : "US"}.M"
...
ids.each_with_index do |id, idx|
  ...
  series_attrs = {
    :xseries_attributes => xs_attrs,
    :name_parts => name_parts,
    :dataPortalName => "FINAL #{regions[idx]}",
    :description => "",
    :unit_id => unit_id,
    :source_id => bls_source_id,
    :source_link => "",
  ...
  # set up load statement
  cpi_us_series = "BUG4#{ids[0]}@US.M"
  eval_str = "BUG4#{id}@#{is_hawaii ? "HI" : "US"}.M"
  eval_str = "(\"#{eval_str}\".ts) / (\"#{cpi_us_series}\".ts)"
  ds_attrs = {
    :eval => eval_str,
    :priority => 50,
    :scale => "1.0",
