...
ids.each_with_index do |id, idx|
  ...
  name_id = name_ids[idx]
  name_parts = {
    :prefix => "#{testing_prefix}CPIRPP#{name_id}I",
    :geography_id => geo_id,
    :freq => "M"
  }
  xs_attrs = {
...
ids.each_with_index do |id, idx|
  ...
  # set up load statement
  cpi_us_series = "#{testing_prefix}CPIRPP#{name_ids[0]}I@US.M"
  eval_str = "#{testing_prefix}CPIRPP#{name_id}I@#{is_hawaii ? "HI" : "US"}.M"
...
ids.each_with_index do |id, idx|
  ...
  name_id = name_ids[idx]
  name_parts = {
    :prefix => "#{testing_prefix}CPIRPP#{name_id}",
    :geography_id => geo_id,
    :freq => "M"
  }
  xs_attrs = {
  ...
  series_attrs = {
    :xseries_attributes => xs_attrs,
    :name_parts => name_parts,
    :dataPortalName => "CALC NONINTERP #{regions[idx]}",
    :description => "",
    :unit_id => unit_id,
    :source_id => bls_source_id,
    :source_link => "",
...
ids.each_with_index do |id, idx|
  ...
  # set up load statement
  cpi_us_series = "#{testing_prefix}CPIRPP#{name_ids[0]}@US.M"
  eval_str = "#{testing_prefix}CPIRPP#{name_id}@#{is_hawaii ? "HI" : "US"}.M"
  eval_str = "((\"#{eval_str}\".ts) / (\"#{cpi_us_series}\".ts)) * 100"
  ds_attrs = {
    :eval => eval_str,
    :priority => 50,
