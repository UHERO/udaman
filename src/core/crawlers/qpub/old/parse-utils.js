    ...
    dataRows.forEach(row => {
        ...
        cells.forEach((cell, index) => {
            if (index < headers.length && headers[index]) {
                ...
                let value = link ? link.getAttribute('href') : cleanText(cell.textContent);
...
export function isAssessedValueField(fieldName) {
    ...
    return assessedValuePatterns.some(pattern => lowerFieldName === pattern || lowerFieldName.endsWith('_' + pattern));
}
...
export function parseLandArea(value) {
    ...
    if (lowerStr.includes('acre')) {
        unit = 'acres';
    } else if (lowerStr.includes('sq') || lowerStr.includes('square feet') || lowerStr.includes('square ft')) {
        unit = 'sqft';
    }
