...
function processDirectory(inputDir, outputDir, condoOnly = false) {
  ...
  if (!outputDir || outputDir === "./parsed-output") {
    ...
    if (islandCode) {
      ...
      if (inputDir.includes(CONFIG.HTML_DIR)) {
        ...
        console.log(
          `Auto-detected island ${islandCode} (${CONFIG.ISLANDS[islandCode]})`,
        );
  ...
  console.log(
    `Found ${htmlFiles.length} HTML files in ${inputDir}${condoOnly ? " (condo-only mode)" : ""}\n`,
  );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (args.length === 0 || args.includes("--help") || args.includes("-h")) {
    console.log(
      "Usage: node parse.js <input-file-or-dir> [output-dir] [options]",
    );
    console.log("\nOptions:");
    console.log(
      "  --condo-only    Only parse condo project files, skip all others",
    );
    console.log("  --help, -h      Show this help message");
    console.log("\nExamples:");
    console.log("  # Parse sample files");
    console.log(
      "  node parse.js sample-html-pages/success/3-9-2-020-005-0000.html",
    );
    ...
    console.log(
      "  node parse.js sample-html-pages/success parsed-output --condo-only",
    );
    console.log(
      "\n  # Parse from NAS (auto-detects island and output directory)",
    );
    console.log(
      `  node parse.js ${path.join(CONFIG.NAS_PATH, CONFIG.HTML_DIR, "1")}`,
    );
    console.log(
      `  node parse.js ${path.join(CONFIG.NAS_PATH, CONFIG.HTML_DIR, "2")}`,
    );
    console.log("\n  # Or specify custom output directory");
    console.log(
      `  node parse.js ${path.join(CONFIG.NAS_PATH, CONFIG.HTML_DIR, "1")} ${path.join(CONFIG.NAS_PATH, CONFIG.JSON_DIR, "1")}`,
    );
