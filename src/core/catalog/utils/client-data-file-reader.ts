/**
 * Browser-safe utility for parsing time series data from 2D string arrays.
 * No Node.js dependencies — can be used client-side or server-side.
 *
...
/** Parse CSV text into a 2D string array, skipping blank rows */
...
class ClientDataFileReader {
  ...
  /** Parse CSV text into a ClientDataFileReader */
  static fromCSV(text: string): ClientDataFileReader {
    return new ClientDataFileReader(parseCSVToRows(text));
  }
...
export default ClientDataFileReader;
