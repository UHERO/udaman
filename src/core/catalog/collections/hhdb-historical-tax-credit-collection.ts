...
import {
  HhdbHistoricalTaxCredit,
  hhdbHistoricalTaxCreditRowToJSON,
  type HhdbHistoricalTaxCreditAttrs,
} from "../models/hhdb-historical-tax-credit";
import type { HhdbHistoricalTaxCreditJSON } from "../models/hhdb-historical-tax-credit";
import type { HhdbListParams, HhdbListResult } from "../types/hhdb";

const SORTABLE = ["tmk", "period", "description", "amount"];
...
export default class HhdbHistoricalTaxCreditCollection {
  ...
  ): Promise<HhdbListResult<HhdbHistoricalTaxCredit>> {
    const { where, qp, sortCol, sortDir, limit, offset } =
      this._buildQuery(params);

    const [countResult, rows] = await Promise.all([
      rawQuery<{ cnt: number }>(
        `SELECT COUNT(*) as cnt FROM historical_tax_credits ${where}`,
        qp,
      ),
  ...
  static async listJSON(
    params: HhdbListParams,
  ): Promise<HhdbListResult<HhdbHistoricalTaxCreditJSON>> {
    const { where, qp, sortCol, sortDir, limit, offset } =
      this._buildQuery(params);
