...
export default function DvwDataPortal() {
  return (
    <main className="mx-20 mt-5 flex size-full flex-col">
      ...
      <div className="mb-8 flex w-full flex-wrap justify-center gap-4 text-center">
        {modules.map((mod, i) => {
          const Icon = DIMENSION_ICONS[mod];
          return (
            <Link key={`${mod}-${i}`} href={`/data/dvw/${mod}`}>
              <Card className="hover:border-dvw flex h-37.5 w-70 cursor-pointer flex-col items-center justify-center rounded-xl border border-gray-200 p-4 shadow-none transition-transform duration-200 ease-in-out hover:-translate-y-1 hover:shadow-lg">
                {Icon && (
