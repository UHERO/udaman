...
import { getHhdbPermits } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbPermitJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbPermits({
      page,
      limit,
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} permit records`
          : tab === "data"
            ? "Loading..."
            : "Permit field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="permits" />}
    </div>
  );
}
