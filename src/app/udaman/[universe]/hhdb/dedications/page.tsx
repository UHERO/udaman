...
import { getHhdbDedications } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbDedicationJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbDedications({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Dedications</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} dedication records`
          : tab === "data"
            ? "Loading..."
            : "Dedication field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="dedications" />}
    </div>
  );
}
