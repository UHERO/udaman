...
import { getHhdbAssessments } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbAssessmentJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbAssessments({
      page,
      limit,
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} assessment records`
          : tab === "data"
            ? "Loading..."
            : "Assessment field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="assessments" />}
    </div>
  );
}
