...
import { getHhdbYardImprovements } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbYardImprovementJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbYardImprovements({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Yard Improvements</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} yard improvement records`
          : tab === "data"
            ? "Loading..."
            : "Yard improvement field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="yard_improvements" />}
    </div>
  );
}
