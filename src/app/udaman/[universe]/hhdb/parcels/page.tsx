...
import { getHhdbParcels } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbParcelJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbParcels({
      page,
      limit,
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} parcel records`
          : tab === "data"
            ? "Loading..."
            : "Parcel field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="parcels" />}
    </div>
  );
}
