...
import { getHhdbAgriculturalAssessments } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbAgriculturalAssessmentJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbAgriculturalAssessments({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Agricultural Assessments</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} agricultural assessment records`
          : tab === "data"
            ? "Loading..."
            : "Agricultural assessment field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="agricultural_assessments" />}
    </div>
  );
}
