...
import { getHhdbHistoricalTaxCredits } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbHistoricalTaxCreditJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbHistoricalTaxCredits({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Historical Tax Credits</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} tax credit records`
          : tab === "data"
            ? "Loading..."
            : "Tax credit field factors"}
      </p>
      ...
      {tab === "factors" && (
        <HhdbFactors tableName="historical_tax_credits" />
      )}
    </div>
  );
}
