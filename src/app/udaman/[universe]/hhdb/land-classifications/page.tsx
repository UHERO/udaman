...
import { getHhdbLandClassifications } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbLandClassificationJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbLandClassifications({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Land Classifications</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} land classification records`
          : tab === "data"
            ? "Loading..."
            : "Land classification field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="land_classifications" />}
    </div>
  );
}
