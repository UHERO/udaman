...
import { getHhdbCommercialDetails } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbCommercialDetailJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbCommercialDetails({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Commercial Improvement Details</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} commercial detail records`
          : tab === "data"
            ? "Loading..."
            : "Commercial detail field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="commercial_improvement_details" />}
    </div>
  );
}
