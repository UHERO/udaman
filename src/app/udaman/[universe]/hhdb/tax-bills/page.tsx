...
import { getHhdbCurrentTaxBills } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbCurrentTaxBillJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbCurrentTaxBills({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Current Tax Bills</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} tax bill records`
          : tab === "data"
            ? "Loading..."
            : "Tax bill field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="current_tax_bills" />}
    </div>
  );
}
