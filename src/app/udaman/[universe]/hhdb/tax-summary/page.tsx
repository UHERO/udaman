...
import { getHhdbHistoricalTaxSummary } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbHistoricalTaxSummaryJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbHistoricalTaxSummary({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Historical Tax Summary</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} tax summary records`
          : tab === "data"
            ? "Loading..."
            : "Tax summary field factors"}
      </p>
      ...
      {tab === "factors" && (
        <HhdbFactors tableName="historical_tax_summary" />
      )}
    </div>
  );
}
