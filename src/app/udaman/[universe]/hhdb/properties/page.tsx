...
import { getHhdbProperties } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbPropertyJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbProperties({
      page,
      limit,
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} properties in the Hawaii Housing Database`
          : tab === "data"
            ? "Loading..."
            : "Property field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="properties" />}
    </div>
  );
}
