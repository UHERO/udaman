...
import { getHhdbAccessoryStructures } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbAccessoryStructureJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbAccessoryStructures({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Accessory Structures</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} accessory structure records`
          : tab === "data"
            ? "Loading..."
            : "Accessory structure field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="accessory_structures" />}
    </div>
  );
}
