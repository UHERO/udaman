...
import { getHhdbAppeals } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbAppealJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbAppeals({
      page,
      limit,
  ...
  return (
    <div>
      ...
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} appeal records`
          : tab === "data"
            ? "Loading..."
            : "Appeal field factors"}
      </p>
      ...
      {tab === "factors" && <HhdbFactors tableName="appeals" />}
    </div>
  );
}
