...
import { getHhdbHistoricalTaxDetails } from "@/actions/hhdb";
...
export default function Page() {
  ...
  const [data, setData] = useState<HhdbHistoricalTaxDetailJSON[]>([]);
  ...
  const fetchData = useCallback(async () => {
    if (tab !== "data") return;
    setLoading(true);
    const result = await getHhdbHistoricalTaxDetails({
      page,
      limit,
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold">Historical Tax Details</h1>
      <p className="text-muted-foreground mb-4 text-sm">
        {tab === "data" && !loading
          ? `${total.toLocaleString()} tax detail records`
          : tab === "data"
            ? "Loading..."
            : "Tax detail field factors"}
      </p>
      ...
      {tab === "factors" && (
        <HhdbFactors tableName="historical_tax_details" />
      )}
    </div>
  );
}
