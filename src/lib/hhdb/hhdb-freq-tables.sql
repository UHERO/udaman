...
-- MLS listings (scraped, not qPublic). Same shape as the rest, with two
-- differences forced by the data: the charset/collation is pinned to
-- mls_listings' own so GROUP BY and this PRIMARY KEY agree on which values are
