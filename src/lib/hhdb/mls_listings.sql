-- MLS listings scraped from public listing sites (src/core/crawlers/mls).
--
-- DURABLE, REMOTE-ONLY tables, like tg_transactions: they are deliberately NOT
-- in hhdb-schema.sql and NOT in ALL_DATA_TABLES (qpub-db-sync.ts), because
-- everything in that list is DROPped and recreated on every qpub rebuild.
-- There is no DROP TABLE here on purpose — first_seen_at and the history
-- timeline cannot be re-scraped.
--
-- The data columns of mls_listings are generated from MLS_COLUMNS in
-- src/core/crawlers/mls/columns.ts; mls-listings-ddl.test.ts fails if this
-- file and that list drift apart. To add a column: add it to MLS_COLUMNS, add
-- it here, and write an ALTER TABLE migration.
--
-- All DATETIMEs are Hawaii wall-clock, stamped by the loader with NOW().
-- Every identifier is backtick-quoted: `view`, `security`, `pool`,
