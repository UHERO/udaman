...
ALTER TABLE `users` MODIFY `role` ENUM('external','fsonly','internal','fellow','mcp-only','admin','dev') NOT NULL DEFAULT 'external';

ALTER TABLE `role_permissions` MODIFY `role` ENUM('external','fsonly','internal','fellow','mcp-only','admin','dev') NOT NULL;
