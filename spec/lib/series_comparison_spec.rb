  ...
  before(:each) do
   ...
   # test wuth multipliers   
    describe "MATCHING DATA with comparison hash" do    
      xit "should declare dates match if dates in series data array match given array exactly" do
      	@visns.match_dates_with(@visns.data).should be_true
      end
    
      xit "should declare dates do no match if given array has more dates, but otherwise contains all dates present in the series data array" do
        data_copy = @visns.data.clone
        data_copy["1881-01-01"] = 2001
        @visns.match_dates_with(data_copy).should be_false
      end
    
      xit "should declare dates do not match if given array is missing any date present in the series data array" do
        data_copy = @visns.data.clone
        data_copy.delete "1981-01-01"
        @visns.match_dates_with(data_copy).should be_false
      end
    
      xit "should declare that data values match for a given date and given array if values are exactly the same" do
        @visns.match_data_date("1981-01-01", @visns.data).should be_true
      end
    
      xit "should declare that data values match for a given date and given array if values are within an acceptable tolerance range" do
        data_copy = @visns.data.clone
      ...
      xit "should declare that data values match for a given date and given array if both values are nil" do
        data_copy = @visns.data.clone
      ...
      xit "should declare that data values do not match for a given date and given array if values are not within an acceptable tolerance range" do
        data_copy = @visns.data.clone
      ...
      xit "should declare that data values do not match for a given date and a given array if values are completely different" do
        data_copy = @visns.data.clone
        data_copy["1981-01-01"] = 999999.1
        @visns.match_data_date("1981-01-01", data_copy).should be_false
      end
    
      xit "should declare that data values do not match for a given date and a given array if values are different classes" do
        data_copy = @visns.data.clone
      ...
      xit "should declare that data values do not match for a given date and a given array if one value is nil and the other is not" do
        data_copy = @visns.data.clone
        data_copy["1981-01-01"] = nil
      ...
      xit "should declare a data array match given a data array if the array is exactly the same" do
        @visns.identical_to?(@visns.data).should be_true
      end
    
      xit "should declare a data array match if data array values match and have the same number of non-nil elements" do
        data_copy = @visns.data.clone
      ...
      xit "should declare a data array mismatch if the given array has more non-nil elements than the series data array" do
        data_copy = @visns.data.clone
        data_copy["1881-01-01"] = 1999
        @visns.identical_to?(data_copy).should be_false
      end
    
      xit "should declare a data array mismatch if the series data array has more non-nil elements than the series data array" do
        data_copy = @visns.data.clone
        @visns.data["1881-01-01"] = 1999
        @visns.identical_to?(data_copy).should be_false
      end
    
      xit "should declare a data array mismatch if the series data array has different values than the given data array" do
        data_copy = @visns.data.clone
        data_copy["1981-01-01"] = 999999.1
        @visns.identical_to?(data_copy).should be_false
      end
