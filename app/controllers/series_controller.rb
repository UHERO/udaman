class SeriesController < ApplicationController
  ...
  def alias_create
    ...
    if mid > 0
      ...
      redirect_to(
        series_path(@series),
        notice: "Alias series successfully created"
      )
    end
  end
    ...
    begin
      ...
      respond_to do |format|
        format.html do
          ...
          redirect_to(
            series_path(@series),
            notice: "Series successfully updated"
          )
        end
        format.xml { head :ok }
        format.js do
          render(js: "window.location.href = '#{series_path(@series)}'")
        end
      end
    rescue => e
  ...
  def do_clip_action
    if params[:clip_action] =~ /csv/
      redirect_to(
        group_export_series_path(
          type: params[:clip_action],
          format: :csv,
          layout: false
        )
      )
  ...
  def meta_store
    ...
    unless fields.count > 0
      redirect_to(
        meta_update_series_path,
        notice: "No fields were specified for metadata propagation"
      )
      return
    end
    new_properties =
      fields
        .map do |f|
          [f, series_params[f] || series_params[:xseries_attributes][f]]
        end
        .to_h
  ...
  def index
    ...
    return if current_user.dbedt?
    unless current_user.internal_user?
      render(
        text: "Your current role only gets to see this page.",
        layout: true
      )
      return
    end
  ...
  def autocomplete_search
    render json:
             Series
               .web_search(params[:term], params[:universe])
               .map { |s|
                 {
  ...
  def new_search(search_string = nil)
    ...
    Rails.logger.info do
      "SEARCHLOG: user #{current_user.username.ljust(9, " ")} searched #{@search_string}"
    end
    all_series =
      Series.search(
        @search_string,
        limit: ENV["SEARCH_DEFAULT_LIMIT"].to_i,
        user: current_user
      )
  ...
  def show(no_render: false)
    @desc =
      begin
        AremosSeries.get(@series.name).description
      rescue StandardError
        "No Aremos Series"
      end
    @vintage =
      begin
        Date.parse(params[:vintage])
      rescue StandardError
        nil
      end
    ...
    @dsas =
      @series.enabled_data_sources.map { |ds| ds.data_source_actions }.flatten
    @clipboarded = current_user.clipboard.include?(@series)
    @dependencies =
      @series.who_depends_on_me(%w[series.name series.id]).sort_by { |a| a[0] }
    return if no_render
  ...
  def no_source_no_restrict
    @series =
      Series
        .get_all_uhero
        .where("source_id is null and restricted = false")
        .order(:name)
        .paginate(page: params[:page], per_page: 50)
  end
  ...
  def quarantine
    @series =
      Series
        .get_all_uhero
        .where("quarantined = true and restricted = false")
        .order(:name)
        .paginate(page: params[:page], per_page: 50)
  end
  ...
  def csv2tsd
    ...
    respond_to do |format|
      format.tsd do
        send_data helpers.do_csv2tsd_convert(@filepath),
                  filename:
                    @filepath.original_filename.change_file_extension(
                      "tsd",
                      nopath: true
                    ),
  ...
  def add_to_quarantine
    @series.add_to_quarantine if @series
  ...
  def remove_from_quarantine
    if @series
      ...
      if params[:next_action].present?
        redirect_to(
          send("#{params[:next_action]}_series_path", @series),
          notice: "Series removed from quarantine"
        )
      else
        redirect_to(
          series_path(@series),
          notice: "Series removed from quarantine"
        ) # Redirect to series#show
      end
    else
  ...
  def destroy
    ...
    redirect_to(
      series_index_path,
      notice: "Series '#{series_name}' was successfully destroyed."
    )
  end
  ...
  def all_tsd_chart
    @all_tsd_files =
      JSON.parse(open("http://readtsd.herokuapp.com/listnames/json").read)[
        "file_list"
      ]
    @all_series_to_chart = []
  ...
  def transform
    eval_string = params[:eval].gsub(%r{([+*/()-])}, ' \1 ').strip ## puts spaces around operators and parens
    eval_statement =
      eval_string
        .split(" ")
        .map { |e| valid_series_name(e) ? %Q{"#{e}".ts} : e }
        .join(" ")
    @series =
      begin
        (Kernel.eval eval_statement)
      rescue StandardError
        nil
      end
    unless @series
    ...
    series_list.map do |s|
      name_parts = s.parse_name
      {
        ...
        source:
          ...
          ) ## need rescue because it's a sort column and could be nil
      }
    end
  end
  ...
  def set_attrib_resource_values(series)
    primary_univ =
      series.has_primary? ? series.primary_series.universe : "UHERO"
    ...
    @all_details =
      SourceDetail.where(universe: primary_univ) if @all_details.empty?
  ...
  def json_from_heroku_tsd(series_name, tsd_file)
    url =
      URI.parse(
        "http://readtsd.herokuapp.com/open/#{tsd_file}/search/#{series_name[0..-3]}/json"
      )
