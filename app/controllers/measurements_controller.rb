class MeasurementsController < ApplicationController
  ...
  def new
    data_list =
      begin
        DataList.find(params[:data_list_id].to_i)
      rescue StandardError
        nil
      end
  ...
  def create
    ...
    if @measurement.save
      data_list =
        begin
          DataList.find(params[:data_list_id])
        rescue StandardError
          nil
        end
      if data_list
        ...
        redirect_to(
          @measurement,
          notice: "Measurement was successfully created."
        )
      end
    else
      render(:new)
  ...
  def import_clip
    current_user
      ...
      .each do |s|
        begin
          @measurement.series.push(s)
        rescue StandardError
          next
        end ## rescue to cover cases where the series is already linked
      end
  ...
  def add_series
    ...
    unless series.universe == @measurement.universe
      redirect_to(
        new_alias_series_path(
          series,
          new_univ: @measurement.universe,
          add_to_meas: @measurement.id
        )
      )
      return
    end
    ...
    if @measurement.series.include? series
      redirect_to(
        edit_measurement_path(@measurement.id),
        notice: "This series is already included!"
      )
      return
    end
  ...
  def propagate
    ...
    unless fields && series
      redirect_to(
        ...
        notice: "Please select at least one field and one series"
      )
      return
    end
    ...
    if fields_to_update.empty?
      redirect_to(
        measurement_path(@measurement),
        notice: "No fields to update were found."
      )
      return
    end
    new_vals_hash =
      fields_to_update
        .map { |f| [translate(f), @measurement.read_attribute(f)] }
        .to_h
    series.keys.each do |name|
      Series.find_by(
        universe: @measurement.universe,
        name: name
      ).update_attributes(new_vals_hash)
    end
    redirect_to(
      @measurement,
      notice:
        "Field(s) " + fields_to_update.join(", ") + " propagated successfully."
    )
  end
