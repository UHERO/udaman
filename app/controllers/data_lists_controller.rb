class DataListsController < ApplicationController
  ...
  before_action :set_data_list, except: %i[index new create]
  ...
  def analyze_view
    ...
    @series_name =
      (
        if params[:list_index].nil?
          params[:series_name]
        else
          @data_list.series_names[params[:list_index].to_i]
        end
      )
  ...
  def new
    category =
      begin
        Category.find(params[:category_id].to_i)
      rescue StandardError
        nil
      end
  ...
  def duplicate
    ...
    new_data_list.save!
    redirect_to(
      edit_data_list_path(new_data_list),
      notice: "Successfully Duplicated Data List"
    )
  end
  ...
  def create
    ...
    respond_to do |format|
      if @data_list.save
        category.update_attributes(data_list_id: @data_list.id) if category
  ...
  def update
    respond_to do |format|
      if @data_list.update! data_list_params.merge(updated_by: current_user.id)
        format.html do
          redirect_to(@data_list, notice: "Data list was successfully updated.")
        end
        format.xml { head :ok }
      else
  ...
  def edit_as_text
    @prefix_list =
      @data_list
        .data_list_measurements
        .order(:list_order)
        .map { |dlm| dlm.measurement.prefix }
        .join("\n")
  end
  ...
  def add_clip
    ...
    redirect_to(
      edit_data_list_url(@data_list),
      notice: "#{count} series added to clipboard"
    )
  end

  def add_measurement
    unless @data_list.add_measurement(
             Measurement.find params[:data_list][:meas_id].to_i
           )
      redirect_to(
        edit_data_list_url(@data_list.id),
        notice: "This Measurement is already in the list!"
      )
      return
    end
    respond_to do |format|
  ...
  def remove_measurement
    respond_to { |format| format.js { head :ok } }
    measurements =
      DataListMeasurement
        .where(data_list_id: @data_list.id)
        .to_a
        .sort_by { |m| m.list_order }
    index_to_remove =
      measurements.index do |m|
        m.measurement_id == params[:measurement_id].to_i
      end
    new_order = 0
    measurements.each_index do |i|
      next if index_to_remove == i
    ...
    id_to_remove =
      DataListMeasurement.find_by(
        ...
        measurement_id: params[:measurement_id].to_i
      ).id
  ...
  def move_measurement_up
    ...
    measurements_array =
      @data_list.data_list_measurements.to_a.sort_by { |m| m.list_order }
    old_index =
      measurements_array.index do |m|
        m.measurement_id == params[:measurement_id].to_i
      end
    return if old_index <= 0
  ...
  def move_measurement_down
    ...
    return if old_index >= measurements_array.length - 1
    measurements_array.each_index do |i|
      if old_index + 1 == i
        measurements_array[i].update list_order: i - 1
        next
  ...
  def set_measurement_indent
    ...
    new_indent =
      params[:indent_in_out] == "in" ? current_indent + 1 : current_indent - 1
    ...
    respond_to do |format|
      format.json do
        render json:
                 '{ "the_indent": "%s" }' %
                   helpers.make_indentation(new_indent),
               status: 200
      end
    end
  ...
  def data_list_params
    params.require(:data_list).permit(
      :name,
      :list,
      :startyear,
      :created_by,
