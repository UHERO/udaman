class DashboardsController < ApplicationController
  ...
  def rerun_job
    job =
      begin
        ReloadJob.find(params[:id].to_i)
      rescue StandardError
        raise("Could not find existing ReloadJob id = #{params[:id]}")
      end
  ...
  def restart_dvwapi
    ...
    `sudo systemctl restart dvw-api.service`
    render(
      json: {
        message: "DVW API restart #{$?.success? ? "done" : "FAIL"}"
      }
    )
  end
  ...
  def clear_api_cache
    if Rails.env.production?
      ...
      render(
        json: {
          message: "NAS file sync #{$?.success? ? "done" : "FAIL"}"
        }
      )
    else
      mylogger :info,
