class DataLoadMailer < ActionMailer::Base
  ...
  def series_refresh_notification(circular_series, stale_data_sources, ds_count, errors)
    begin
      ...
      @errors = errors
      mail(:to => ["jrpage@hawaii.edu", "james29@hawaii.edu", "ashleysh@hawaii.edu"], :subject => "[Udamacmini Series Reload] #{ds_count} Datasources loaded : #{@errors.count} / #{@circular_series.count} / #{@stale_data_sources.count} ")
    rescue => e
