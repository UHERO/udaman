class PackagerMailer < ActionMailer::Base
  ...
  def rake_notification(rake_task, download_results, errors, series, output_path, is_error)
    begin
      ...
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => subject)
    rescue => e
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.rake_notification error", :body => e.message, :content_type => "text/plain")
    end
  end
  ...
  def rake_error(e, output_path)
    begin
      @error = e
      @output_path = output_path
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "Rake failed in an unexpected way (Udamacmini)")
    rescue => e
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.rake_error error", :body => e.message, :content_type => "text/plain")
    end
  end
  ...
  def visual_notification
    begin
      ...
      recipients = ["jrpage@hawaii.edu", "james29@hawaii.edu", "fuleky@hawaii.edu", "ashleysh@hawaii.edu", "diliaur@hawaii.edu"]
      ...
      mail(:to => recipients, :subject => subject)
        ...
        puts e.message
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.visual_notification error", :body => e.message, :content_type => "text/plain")
    end
  end
  ...
  def download_link_notification(handle = nil, url = nil, save_path = nil, created = false)
    begin
      ...
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.download_link_notification error", :body => e.message, :content_type => "text/plain")
    end
  end
  ...
  def website_post_notification(post_name, post_address, new_data_series, created)
    begin
      ...
      mail(:to => ["jrpage@hawaii.edu", "james29@hawaii.edu", "diliaur@hawaii.edu"], :subject => subject) #{})
    rescue => e
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.website_post_notification error", :body => e.message, :content_type => "text/plain")
    end
  end
  ...
  def prognoz_notification(recipients, send_edition)
    begin
      ...
      mail(:to => ["jrpage@hawaii.edu", "diliaur@hawaii.edu"], :subject => "[UDAMACMINI] PackageMailer.prognoz_notification error", :body => e.message, :content_type => "text/plain")
    end
  end
end
