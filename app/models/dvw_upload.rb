class DvwUpload < ApplicationRecord
  ...
  def store_upload_files(series_file)
    ...
    begin
      ...
      write_file_to_disk(filename, series_file_content) or
        raise StandardError, "DVW upload disk write failed"
      ...
      **{ col_sep: "\t", headers: true, return_headers: true }
      ...
      unless columns
        columns =
          row_pairs
            .to_a
            .reject { |x, _| x.blank? || x =~ /^\s*[lo]_/i }
            .map { |x, _| x.strip.downcase } ## leave out L_* and O_*
        ...
        columns.each do |c|
          if c =~ /\W/
            raise("Illegal character in #{dimension} column header: #{c}")
          end
        end
        next
      end
      ...
      parent_set.push [row["parent"], row["handle"]] if row["parent"]
          ...
          columns.each do |col|
            row_values.push case col
                            when "module"
                              mod
                            when "header"
                              (row["data"].to_s == "0" ? 1 : 0) ## semantically inverted
                            when "level"
                              level
                            when "order"
                              order
  ...
  def load_series_csv
    ...
    dp_data_set.in_groups_of(1000, false) { |dps| db_execute_set sql_stmt, dps }
  ...
  def worker_tasks(do_csv_proc: false)
    ...
    if Rails.env.production?
      output = `ssh uhero12.colo.hawaii.edu "bin/clear_api_cache.sh /dvw/"`
      if $?.success?
        mylogger :info,
                 "worker_tasks: API /dvw/ cache clear: SUCCESS, #{output.to_i} entries cleared"
      else
