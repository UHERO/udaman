class NewDbedtUpload < ApplicationRecord
  ...
  def load_meta_csv
    ...
    root_cat =
      begin
        Category.find_by(universe: "DBEDT", ancestry: nil).id.to_s
      rescue StandardError
        raise("No DBEDT root category found")
      end

    CSV.foreach(
      csv_path,
      "r",
      **{ col_sep: "\t", headers: true, return_headers: false }
    ) do |pairs|
      row = {}
      pairs.to_a.each do |header, data| ## convert row to hash keyed on column header, force blank/empty to nil
        ...
        row[header.strip.downcase] = (
          begin
            Integer(val)
          rescue StandardError
            val
          end
        ) ## convert integers to Integer type if possible
      end
      ...
      if row["unit"] ####################### data_list, measurements entry
        ...
        dlm =
          DataListMeasurement.find_by(
            data_list_id: data_list.id,
            measurement_id: measurement.id
          )
        dlm.update(list_order: row["order"]) if dlm
        mylogger :debug,
                 "added measurement #{measurement.prefix} to data_list #{data_list.name}"
      end
    end
    ...
    CSV.foreach(
      ...
      **{ col_sep: "\t", headers: true, return_headers: false }
      ...
      pairs.to_a.each do |header, data| ## convert row to hash keyed on column header, force blank/empty to nil
        ...
        ) ## convert numeric types if possible. Parens are crucial!
      end

      ind_id =
        row["ind_id"] ||
          raise("Blank indicator ID around row #{data_points.count}")
      ...
      geo_handle =
        [nil, "HI", "HAW", "HON", "KAU", "MAU"][area] ||
          raise(
            "Area ID=#{area} is blank/unknown around row #{data_points.count}"
          )
      geo_id =
        begin
          allgeos[geo_handle].id
        rescue StandardError
          nil
        end
      unless geo_id
        allgeos[geo_handle] = Geography.get(
          universe: "DBEDT",
          handle: geo_handle
        ) || raise("Area handle #{geo_handle} missing from db")
        ...
        current_measurement =
          Measurement.find_by(universe: "DBEDT", prefix: prefix) ||
            Measurement.create(
              universe: "DBEDT",
              prefix: prefix,
              data_portal_name: ind_meta["indicatorfortable"]
            )
      end
      ...
      if current_series.nil? || current_series.name != name
        ...
        if source_str && source_str.downcase != "none"
          ...
          unless source_id
            allsources[source_str] = begin
              Source.get_or_new(source_str, nil, "DBEDT").id
            rescue StandardError
              raise("Failed to create Source #{source_str}")
            end
            source_id = allsources[source_str]
          end
        end
        ...
        if unit_str.downcase != "none"
          ...
          unless unit_id
            allunits[unit_str] = begin
              Unit.get_or_new(unit_str, "DBEDT").id
            rescue StandardError
              raise("Failed to create Unit #{unit_str}")
            end
            unit_id = allunits[unit_str]
          end
        end
        ...
        if current_series
          ...
          current_data_source =
            DataSource.find_by(universe: "DBEDT", series_id: current_series.id)
        else
          current_series =
            Series.create_new(
              universe: "DBEDT",
      ...
      data_points.push(
        {
          xs_id: current_series.xseries_id,
  ...
  def worker_tasks(do_csv_proc: false)
    ...
    self.transaction do
      ...
      self.update_attributes(
        ...
        active: true,
        last_error_at: nil,
        last_error: "#{total} data points loaded"
      )
    end
    mylogger :info, "worker_tasks: loaded and active"

    mylogger :info, "worker_tasks: starting DataPoint.update_public_data_points"
    DataPoint.update_public_data_points("DBEDT") ||
      raise("FAILED to update public data points")

    if Rails.env.production?
      output = `ssh uhero12.colo.hawaii.edu "bin/clear_api_cache.sh /v1/"`
      if $?.success?
        mylogger :info,
                 "worker_tasks: API /v1/ cache clear: SUCCESS, #{output.to_i} entries cleared"
      else
        mylogger :warn, "worker_tasks: API /v1/ cache clear FAIL: #{$?}"
      end
    else
      mylogger :info,
               "worker_tasks: Skipping API cache clear in #{Rails.env} environment"
    end
  end
  ...
  def csv_extract
    ...
    Dir
      .glob(File.join(csv_path, "*.csv"))
      .each { |f| File.rename(f, f.downcase) } ## force csv filenames to lower case
