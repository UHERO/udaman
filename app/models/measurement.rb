class Measurement < ApplicationRecord
  ...
  def replace_all_series(new_s_list, from_universe = self.universe)
    my_series =
      series.includes(:measurement_series) ## eager load the bridge table
    ...
    self.transaction do
      ...
      new_s_list.each do |new|
        next if new == "_done"
        series =
          Series.find_by(universe: from_universe, name: new) ||
            raise("Unknown #{from_universe} series name #{new}")
        begin
          (self.series << series)
        rescue StandardError
          raise("Series #{new} duplicated?")
        end
      end
    end
  end
  ...
  def Measurement.deep_copy_to_universe(
    prefix_list,
    universe,
    name_trans_f = nil,
    properties = {}
    ...
    prefix_list.each do |p|
      ...
      begin
        m =
          Measurement.find_by(universe: "UHERO", prefix: p) ||
            raise("UHERO measurement #{p} not found")
  ...
  def duplicate(universe, name_trans_f = nil, properties = {})
    ...
    if universe == self.universe
      raise "Cannot duplicate #{self} into same universe #{universe}"
    end
    new_name = name_trans_f ? name_trans_f.call(prefix) : prefix
    if Measurement.find_by(universe: universe, prefix: new_name)
      raise(
        "Cannot duplicate because #{new_name} already exists in #{universe}"
      )
    end
    include_series = properties.delete(:deep_copy)
    new_m = self.dup
    new_m.assign_attributes(
      properties.merge(universe: universe, prefix: new_name)
    )
    new_m.save!
    if include_series
      series.each do |s|
        ...
        begin
          ...
          raise("Series #{ali_s} link to Meas #{new_m} duplicated?")
        end
      end
    end
  end
