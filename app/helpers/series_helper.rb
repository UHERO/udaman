module SeriesHelper
  ...
  def series_group_export(type, series)
    case type
    when "metacsv"
      series_meta_csv_gen(series)
    when "datacsv"
      series_data_csv_gen(series)
    when "datatsd"
      series_data_tsd_gen(series)
    else
      raise("series_group_export: unknown type #{type}")
    end
  end
    ...
    CSV.generate(nil, col_sep: "\t") do |csv|
      ...
      series_set.each do |s|
        csv << columns.map do |col|
          ...
          begin
            subattr.nil? ? s.send(attr) : s.send(attr).send(subattr)
    ...
    series_set.each do |s|
      begin
        ...
        Rails.logger.error do
          "series_data_tsd_gen: to_tsd conversion failure for #{s}: #{e.message}"
        end
      end
    end
    output
  ...
  def google_charts_data_table
    ...
    dates_array.each do |date|
      ...
      rs +=
        sorted_names
          .map { |s| series_data[s][date] == "" ? "0" : series_data[s][date] }
          .join(", ") + "],\n"
    end
    rs += "]);\n"
  end
    ...
    parts.each_with_index do |str, index|
      ...
      when :nondate
        ...
        if download
          ...
          parts[
            index
          ] = '<span class="error_message" title="Non-existent download!">%s</span>' %
            parts[index]
        end
      when :date
        parts[index] = link_to(
          str,
          { controller: :downloads, action: :by_pattern, pat: str }
        )
      else
    ...
    words.each_with_index do |word, index|
      ...
      if word =~ /^<(.*)>$/
        ...
        if valid_data_path(rel_path)
          words[index] = link_to(
            word,
            { controller: :downloads, action: :pull_file, path: rel_path }
          )
        end
        next
      end
  ...
  def sa_indicator(string)
    ...
    short =
      {
        adjustment_not_applicable: "NA",
        adjustment_seasonally_adjusted: "SA",
        adjustment_not_seasonally_adjusted: "NS"
      }[
        sa_sym
      ] || "NA"
    if short == "NA"
      ...
      "<span class='#{short.downcase}-indicator'>#{short}</span>".html_safe
    end
  end
  ...
  def make_alt_universe_links(series)
    ...
    series
      .aliases
      .sort_by { |x| [x.is_primary? ? 0 : 1, x.universe] }
      .each do |s|
        links.push link_to(
                     universe_label(s),
                     { controller: :series, action: :show, id: s.id },
                     title: s.name
                   )
    ...
    if series.is_primary? && alt_univs[series.universe] &&
      ...
      alt_univs[series.universe].each do |univ|
        ...
        links.push link_to(
                     univ_gray_create_label(univ),
                     new_alias_series_path(series, new_univ: univ),
                     title: "Create new"
                   )
      end
    end
  ...
  def universe_label(series)
    if series.is_primary?
      "<span class='primary_series'>#{series.universe}</span>".html_safe
    else
      series.universe
    end
  end
