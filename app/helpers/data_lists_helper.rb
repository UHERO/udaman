module DataListsHelper
  ...
  def csv_helper_data_list
    CSV.generate do |csv|
      ...
      dates_array.each do |date|
        csv << [date] +
          names.map { |series_name| series_data[series_name][date] }
      end
    end
  end
  ...
  def js_data_raw
    ...
    sorted_names.each do |s|
      series_data_arrays[s] = dates_array.map do |date|
        series_data[s][date].to_s
      end
    end
    rs = ""
    rs += "{\n"
    ...
    rs += "\"data\" : {\n"
    sorted_names.each do |s|
      rs += "\"" + s + "\" : [" + series_data_arrays[s].join(", ") + "],\n"
    end
    rs += "}\n"
    rs += "}\n"
    rs
  ...
  def google_charts_data_table_list
    ...
    dates_array.each do |date|
      rs +=
        "['" + date.strftime("%Y-%m-%d") + "'," +
          sorted_names
            .map { |s| series_data[s][date].nil? ? 0 : series_data[s][date] }
            .join(", ") + "],\n"
    end
    rs += "]);\n"
    rs
  ...
  def make_indentation(n)
    ...
    (1..n)
      .map { "<i class='fas fa-minus' aria-hidden='true'></i> " }
      .join
      .html_safe
  end
  ...
  def generate_filter_controls(geo, freq, sa)
    html =
      select_tag(
        ...
        options_from_collection_for_select(
          Geography.where(universe: "UHERO").order(:id),
          :handle,
          :handle_with_name,
          geo
        )
    ...
    html +
      select_tag(
        "adjustment_seasonally_adjusted",
        options_for_select(
          %w[
            all
            adjustment_seasonally_adjusted
            adjustment_not_seasonally_adjusted
