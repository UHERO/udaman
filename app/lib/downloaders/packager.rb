module Downloaders
  class Packager
    def Packager.temp
      const_q = {
        "KNRSDNS@HON.Q" =>
          %Q|Series.load_from_download  "QSER_G@hawaii.gov", { :file_type => "xls", :start_date => "1993-01-01", :sheet => "G-25", :row => "block:6:1:4", :col => "repeat:2:5", :frequency => "Q" }|,
        "KNRSDNS@HAW.Q" =>
          %Q|Series.load_from_download  "QSER_G@hawaii.gov", { :file_type => "xls", :start_date => "1993-01-01", :sheet => "G-26", :row => "block:5:1:4", :col => "repeat:2:5", :frequency => "Q" }|,
        "PICTCONNS@HON.Q" =>
          %Q|Series.load_from_download  "QSER_E@hawaii.gov", { :file_type => "xls", :start_date => "1982-01-01", :sheet => "E-7", :row => "block:6:1:4", :col => "repeat:2:5", :frequency => "Q" }|
      }
    ...
    def changed?
      download_results.each { |handle, hash| return true if hash[:changed] }
      return false
    end
    ...
    def download_results_string
      ...
      download_results.each do |handle, hash|
        results_string +=
          "#{handle.rjust(30, " ")} | #{hash[:changed] ? "c" : " "} | Success\n" if hash[
          :status
        ] == 200
        results_string +=
          "#{handle.rjust(30, " ")} |   | Error (#{hash[:status]})\n" unless hash[
          :status
        ] == 200
    ...
    def series_summary_string
      ...
      date2 = sorted[-2]
      date1 = sorted[-3]
      if date1.nil? or date2.nil? or date3.nil?
        return "Not enough data to display"
      end
      results_string = "DOWNLOADED SERIES:\n-------------------\n"
      results_string +=
        "#{"".rjust(20, " ")} | #{date1.rjust(10, " ")} | #{date2.rjust(10, " ")} | #{date3.rjust(10, " ")}\n"
      @series.each do |series, data|
        ...
        results_string +=
          "#{series.rjust(20, " ")} | #{d1.rjust(10, " ")} | #{d2.rjust(10, " ")} | #{d3.rjust(10, " ")}\n"
      end
      results_string
    end
    ...
    def write_definitions_to(output_path) # = "udaman")
      begin
        ...
        changed =
          Series.write_data_list @series.keys,
                                 @output_path unless @definitions.nil?
    ...
    def send_output_email
      ...
      PackagerMailer.rake_notification(
        job_name,
        download_results,
        errors,
        @series,
        @output_path,
        (errors != [] or download_problem?)
      ).deliver
    end
    ...
    def get_data_from_definitions
      ...
      @definitions.each do |series_name, definition|
        ...
        begin
          ...
          series[series_name] = {}
          @errors.push(
            { series: series_name, definition: definition, error: e.message }
          )
        end
      end
