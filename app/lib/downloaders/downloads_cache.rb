module Downloaders
  class DownloadsCache
    def initialize(options = nil)
      ...
      if ds_id
        @data_source =
          DataSource.find(ds_id) ||
            raise("No definition with id='#{ds_id}' found")
      end
    end

    def setup_and_check(type, handle, path = nil, skip_proc = false)
      Rails.logger.debug do
        "... Entered method setup_and_check: type=#{type}, handle=#{handle}, path=#{path}"
      end
      skip = false
      if path.nil? ## this means that handle != :manual
        @dload =
          @cache[:dloads][handle] || Download.get(handle) ||
            raise("handle '#{handle}' does not exist")
        @cache[:dloads][handle] = @dload
        @handle = handle
        path =
          (
            if @dload.extract_path_flex.blank?
              @dload.save_path_flex
            else
              @dload.extract_path_flex
            end
          )
        ...
        if @data_source && skip_proc && !@skip_override
          ...
          dsd =
            @cache[:dsds][bridge_key] ||
              DataSourceDownload.get_or_new(@data_source.id, @dload.id)
          @cache[:dsds][bridge_key] = dsd

          skip =
            @dload.last_change_at <= dsd.last_file_vers_used &&
              @evalhash == dsd.last_eval_options_used
        end
      end
      @path = path
      skip
    ...
    def mark_handle_used(handle)
      ...
      @cache[:used_dloads][handle] = @cache[:dloads][handle] ||
        raise("No download handle='#{handle}' to mark")
    end
    ...
    def update_last_used
      ...
      @cache[:used_dloads].values.each do |dload|
        ...
        dsd.update last_file_vers_used: dload.last_change_at,
                   last_eval_options_used: @evalhash
      end
    end

    def xls(handle, sheet, path = nil, date = nil, skip_proc = false)
      Rails.logger.debug do
        "... Entered method xls: handle=#{handle}, sheet=#{sheet}, path=#{path}"
      end
      path &&= File.join(ENV["DATA_PATH"], path)
      skip = setup_and_check("xls", handle, path, skip_proc)
      sheet = @dload.sheet_override.strip if @dload &&
        !@dload.sheet_override.blank?
    ...
    def set_xls_sheet(sheet, date)
      Rails.logger.debug do
        "... Entered method set_xls_sheet: sheet=#{sheet}, date=#{date}"
      end
      excel =
        begin
      ...
      def_sheet =
        case
        when sheet_parts[0] == "sheet_num"
          excel.sheets[sheet_parts[1].to_i - 1]
        when sheet_parts[0] == "sheet_name" && sheet_parts[1].upcase == "M3"
          get_month_name(date)
        when sheet =~ /\[or\]/i
          sheetnames = sheet.split(/\[or\]/i).collect! { |s| s.strip.downcase }
          index =
            excel.sheets.index { |s| sheetnames.include?(s.strip.downcase) }
      ...
      begin
        ...
        raise 'no sheet matching "%s" found in file %s [handle=%s]' %
                [sheet, @dload.save_path_flex, @handle]
      end
      sheet_key = make_cache_key("xls", @path, sheet)
    ...
    def csv(handle, path = nil, skip_proc = false)
      Rails.logger.debug do
        "... Entered method csv: handle=#{handle}, path=#{path}"
      end
      path &&= File.join(ENV["DATA_PATH"], path)
    ...
    def download_handle(force = false)
      Rails.logger.debug do
        "... Entered method download_handle: @handle=#{@handle}"
      end
      t = Time.now
      unless force
        ## no redownload if very recent
        if @dload.last_download_at && @dload.last_download_at > (t - 1.hour)
          return nil
        end
      end
      ...
      Rails.logger.info do
        "#{Time.now - t} | cache miss: downloaded #{@handle}"
      end
    ...
    def alternate_fastercsv_read(path)
      ...
      while (line = csv_file.gets)
        begin
          ...
          Rails.logger.debug do
            "alternate_fastercsv_read #{path}: Problem with the following line\n#{line}"
          end
          csv_data.push([])
        end
      end
