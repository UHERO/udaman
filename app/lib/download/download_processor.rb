require "roo"
require "csv"

# Error handling in download processor calls
# --------------------------------------------
# line level calls should always raise descriptive errors.
# Errors can be rescued by Packager or Front End
# as necessary

module Download
  class DownloadProcessor
    def initialize(handle, options)
    if options[:file_type].nil?
      raise "File type must be specified when initializing a Download Processor"
    end
    if handle == :manual and options[:path].nil?
      raise "File path must be specified when initializing a manual Download Processor"
    end
    @cached_files = Download::DownloadsCache.new(options)
    @handle = handle
    @options = options
    @file_type = options[:file_type]
    unless %w[txt csv xls xlsx].include?(@file_type)
      raise "not a valid file type option"
    end
    @spreadsheet =
      Download::CsvFileProcessor.new(
        handle,
        options,
        parse_date_options,
        @cached_files
      ) if @file_type == "csv" and validate_csv
    @spreadsheet =
      Download::XlsFileProcessor.new(
        handle,
        options,
        parse_date_options,
        @cached_files
      ) if (@file_type == "xls" or @file_type == "xlsx") and validate_xls
  end

  def get_data
    if @file_type == "txt"
      return Download::TextFileProcessor.new(@handle, @options, @cached_files).get_data
    end
    get_data_spreadsheet
  end

  def get_data_spreadsheet
    raise "spreadhseet was never set" if @spreadsheet.nil?
    index = 0
    data = {}
    begin
      data_point = @spreadsheet.observation_at index
      skip = data_point.class == Hash ? data_point.delete(:skip) : true
      data.merge!(data_point) unless skip
      Rails.logger.debug do
        (skip ? "----- SKIPPED " : "+++++ MERGED ") +
          "DATA POINT :::: #{data_point.to_s}"
      end
      index += 1
    end until data_point.class == String ||
      (@options[:end_date] && data_point.keys[0] == @options[:end_date])
    @cached_files.update_last_used
    data
  end

  # these two processes are a little weird because they require the handle and other parts of the
  # pattern that the date processor itself would not generally need. might not be the best object
  # design...
  def parse_date_options
    date_info = {}
    start_date =
      @options[:start_date] ||
        read_date_from_file(@options[:start_row], @options[:start_col])
    date_info[:start] = adjust_for_frequency(start_date)
    date_info[:rev] = @options[:rev] ? true : false
    date_info
  end

  def adjust_for_frequency(date_string)
    date_parts = date_string.split("-")
    #need cases for annual and quarterly too. For now just doing monthly
    #this should be written in as tests
    return "#{date_parts[0]}-#{date_parts[1]}-01" if @options[:frequency] == "M"
    date_string
  end

  def read_date_from_file(start_row, start_col)
    #assumption is that these will not be files with dates to process. just static file and sheet strings
    #assuming that date is a recognizable format to ruby
    date_cell = nil
    date_cell =
      @cached_files.csv(@handle)[start_row - 1][start_col - 1] if @file_type ==
      "csv"
    date_cell =
      @cached_files.xls(@handle, @options[:sheet]).cell(
        start_row,
        start_col
      ) if @file_type == "xls"
    return date_cell.to_s if date_cell.class == Date
    if date_cell.class == String and !date_cell.index(".").nil?
      return(
        Date.new(
          date_cell.split(".")[0].to_i,
          date_cell.split(".")[1].to_i,
          1
        ).to_s
      )
    end
    if date_cell.class == Float
      return(
        Date.new(
          date_cell.to_s.split(".")[0].to_i,
          date_cell.to_s.split(".")[1].to_i,
          1
        ).to_s
      )
    end
    Date.parse(date_cell).to_s
  end

  def validate_csv
    unless !date_ok or @options[:row].nil? or @options[:col].nil? or
             @options[:frequency].nil?
      return true
    end
    error_string = ""
    error_string += "start date information, " unless date_ok
    error_string += "row specification, " if @options[:row].nil?
    error_string += "column specification, " if @options[:col].nil?
    error_string += "frequency specification, " if @options[:frequency].nil?
    raise "incomplete Download Processor specification because the following information is missing: " +
            error_string.chop.chop
  end

  def date_ok
    if @options[:start_date].nil? and
         (@options[:start_row].nil? or @options[:start_col].nil?)
      return false
    end
    true
  end

  def validate_xls
    unless !date_ok or @options[:row].nil? or @options[:col].nil? or
             @options[:sheet].nil? or @options[:frequency].nil?
      return true
    end
    error_string = ""
    error_string += "start date information, " unless date_ok
    error_string += "row specification, " if @options[:row].nil?
    error_string += "column specification, " if @options[:col].nil?
    error_string += "sheet specification, " if @options[:sheet].nil?
    error_string += "frequency specification, " if @options[:frequency].nil?
    raise "incomplete Download Processor specification because the following information is missing: " +
            error_string.chop.chop
    end
  end
end
