module SeriesSeasonalAdjustment
  ...
  def apply_seasonal_adjustment(factor_application)
    ...
    new_transformation("Applied #{factor_application} Seasonal Adjustment against #{ns_series.name}", adjusted_data)
  end
  
  def apply_ns_growth_rate_sa
    ns_series = self.find_ns_series || raise("no NS series for #{name}")
    adjusted_series = (ns_series.annualized_percentage_change / 100 + 1) * self.shift_forward_years(1)
    new_transformation("Applied Growth Rate Based Seasonal Adjustment against #{ns_series.name}", adjusted_series.data)
  end
