module SeriesArithmetic
  ...
  def faster_change(id)
    ...
    SELECT t1.date, t1.value, (t1.value - t2.last_value) /
      (select coalesce(units, 1) from series_v where id = #{id} limit 1) AS value_change
      FROM (
        SELECT `date`, `value`, (@row := @row + 1) AS row
		    FROM data_points d JOIN xseries x ON x.id = d.xseries_id
          CROSS JOIN (SELECT @row := 0) AS init
		    WHERE x.primary_series_id = #{id} AND `current` = 1 ORDER BY `date`
      ) AS t1
      LEFT JOIN (
  ...
  def faster_yoy(id)
    ...
    sql = <<~SQL
      ...
      FROM (
        ...
        FROM data_points d JOIN xseries x ON x.id = d.xseries_id
        WHERE x.primary_series_id = #{id} AND `current` = 1
      ) AS t1
      LEFT JOIN (
  ...
  def faster_ytd(id)
    ...
    sql = <<~SQL
      ...
      FROM (
        SELECT `date`, `value`, (@sum := IF(@year = YEAR(`date`), @sum, 0) + `value`) AS ytd,
            @year := year(`date`), DATE_SUB(`date`, INTERVAL 1 YEAR) AS last_year
        FROM data_points d JOIN xseries x ON x.id = d.xseries_id
          CROSS JOIN (SELECT @sum := 0, @year := 0) AS init
        WHERE x.primary_series_id = #{id} AND `current` = 1 ORDER BY `date`
      ) AS t1
      LEFT JOIN (
        SELECT `date`, (@sum := IF(@year = YEAR(`date`), @sum, 0) + `value`) AS last_ytd,
            @year := year(`date`)
        FROM data_points d JOIN xseries x ON x.id = d.xseries_id
          CROSS JOIN (SELECT @sum := 0, @year := 0) AS init
        WHERE x.primary_series_id = #{id} AND `current` = 1 ORDER BY `date`
  ...
  def faster_scaled_yoy_diff(id)
    ...
    sql = <<~SQL
      SELECT t1.value, t1.date, (t1.value - t2.last_value) /
        (select coalesce(units, 1) from series_v where id = #{id} limit 1) AS yoy_diff
      FROM (
        SELECT `value`, `date`, DATE_SUB(`date`, INTERVAL 1 YEAR) AS last_year
        FROM data_points d JOIN xseries x ON x.id = d.xseries_id
        WHERE x.primary_series_id = #{id} AND `current` = 1
