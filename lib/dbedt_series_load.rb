module DbedtSeriesLoad
  ...
  def DbedtSeriesLoad.slurp_csv(path)
    ...
    CSV.foreach(path, {col_sep: "\t", headers: true, return_headers: false}) do |row|
      ...
      this_series.data_points.create(date: DbedtSeriesLoad.get_date(row[5], row[6]), value: row[7])
    end
  end

  def DbedtSeriesLoad.get_geo_code(name)
    trans_hash = {
      'Hawaii County' => 'HAW',
      'Honolulu County' => 'HON',
      'Maui County' => 'MAU',
  ...
  def DbedtSeriesLoad.get_date(year, qm)
    if qm =~ /^M(\d+)/i
      "#{year}-#{$1}-01"
    elsif qm =~ /^Q(\d+)/i
