...
task :encachitize_rest_api => :environment do
  ...
  uh_cats = Category.where(%q{universe = 'UHERO' and data_list_id is not null and not (hidden or masked)})
  uh_cats.each do |cat|
  ...
  coh_cats = Category.where(%q{universe = 'COH' and data_list_id is not null and not (hidden or masked)})
